                ...
                <div class="modify-box">
                    ...
                    <div v-if="currStep===0" class="check-box">
                        ...
                        <div class="authcode-box">
                            <span>验证码:</span>
                            <div class="auth-code"><input v-model="authCode" type="text" placeholder="请输入手机验证码">
                    ...
                    <div v-else class="reset-box">
                        <div>
                            <span>新密码：</span><input v-model="password" type="password" placeholder="请输入密码">
                        </div>
                        <div>
                            <span>确认密码：</span><input v-model="repassword" class="confrim-input right" type="password" placeholder="请再次输入密码">
                        </div>
                        <div class="reset-action">
                            <button @click="checkIdent">提交</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
import { getOTP, forgetPassword, editPassword } from "../../api/user";
...
export default {
  ...
  methods: {
    ...
    getAuthCode() {
      ...
      getOTP({
        phone: this.phone,
        type: constants.OTP_TYPE_RESETPWD,
    ...
    checkIdent() {
      ...
      if (
        ...
        this.$message.error("请输入密码与确认密码，两者需相同");
        return;
      }
      forgetPassword({
        phone: this.phone,
        verficode: this.authCode
      })
        .then(res => {
          return editPassword({
            password: aesEncode(
